<template>
    <div class="sintomas-container">
      ...
      <!-- Conteúdo principal -->
      <div class="content">
        <h1 class="header-text">Adicionar Conteúdo para Sintomas</h1>
  
        <!-- Input para upload -->
        <input 
          type="file" 
          @change="onFileChange" 
          accept="video/sintomas/*" 
          class="file-input" 
        />
  ...
  export default {
    ...
    setup() {
      ...
      const uploadVideo = async () => {
        ...
        const storageReference = storageRef(storage, `videos/sintomas/${selectedFile.value.name}`);
      ...
      const fetchVideos = async () => {
        const storageRefVideos = storageRef(storage, 'videos/sintomas');
        try {
      ...
      const updateVideo = async (event) => {
        ...
        try {
          ...
          const storageReference = storageRef(storage, `videos/sintomas/${newFile.name}`);
