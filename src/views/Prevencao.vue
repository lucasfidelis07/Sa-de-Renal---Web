<template>
    <div class="prevencao-container">
      ...
      <!-- Conteúdo principal -->
      <div class="content">
        <h1 class="header-text">Adicionar Conteúdo para Prevenção</h1>
  
        <!-- Input para upload -->
        <input 
          type="file" 
          @change="onFileChange" 
          accept="video/prevencaodadrc/*" 
          class="file-input" 
        />
  ...
  export default {
    ...
    setup() {
      ...
      const uploadVideo = async () => {
        ...
        const storageReference = storageRef(storage, `videos/prevencaodadrc/${selectedFile.value.name}`);
      ...
      const fetchVideos = async () => {
        const storageRefVideos = storageRef(storage, 'videos/prevencaodadrc');
        try {
      ...
      const updateVideo = async (event) => {
        ...
        try {
          ...
          const storageReference = storageRef(storage, `videos/prevencaodadrc/${newFile.name}`);
