<template>
  <div class="fisiologia-container">
    <!-- Cabeçalho fixo -->
    <header class="header">
    ...
    <!-- Conteúdo principal -->
    <div class="content">
      <h1 class="header-text">Adicionar Conteúdo para Fisiologia Renal</h1>

      <!-- Input para upload -->
      <input 
        type="file" 
        @change="onFileChange" 
        accept="video/fisiologiarenal/*" 
        class="file-input" 
      />
...
export default {
  ...
  setup() {
    ...
    const uploadVideo = async () => {
      ...
      const storageReference = storageRef(storage, `videos/fisiologiarenal/${selectedFile.value.name}`);
    ...
    const fetchVideos = async () => {
      const storageRefVideos = storageRef(storage, 'videos/fisiologiarenal');
      try {
    ...
    const updateVideo = async (event) => {
      ...
      try {
        ...
        const storageReference = storageRef(storage, `videos/fisiologiarenal/${newFile.name}`);
