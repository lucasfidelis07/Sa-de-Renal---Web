<template>
  <div class="anatomia-container">
    ...
    <!-- Conteúdo principal -->
    <div class="content">
      <h1 class="header-text">Adicionar Conteúdo para Anatomia Renal</h1>

      <!-- Input para upload -->
      <input 
        type="file" 
        @change="onFileChange" 
        accept="video/anatomiarenal/*" 
        class="file-input" 
      />
...
export default {
  ...
  setup() {
    ...
    const uploadVideo = async () => {
      ...
      const storageReference = storageRef(storage, `videos/anatomiarenal/${selectedFile.value.name}`);
    ...
    const fetchVideos = async () => {
      const storageRefVideos = storageRef(storage, 'videos/anatomiarenal');
      try {
    ...
    const updateVideo = async (event) => {
      ...
      try {
        ...
        const storageReference = storageRef(storage, `videos/anatomiarenal/${newFile.name}`);
